...
class ExpansionTileSampleState extends State<ExpansionTileSample> {
  ...
  List<GlobalKey<AppExpansionTileState>> listOfExpansion;
  String foos = 'One';
  List<String> months = [
    "Januari",
    "Februari",
  ...
  int defaultExpandedItem = 0;

  @override
  void initState() {
    super.initState();
    listOfExpansion = months
        .map((e) => GlobalKey<AppExpansionTileState>(debugLabel: e))
        .toList();
  }
        ...
        body: ListView.builder(
            ...
            itemBuilder: (context, index) {
              return AppExpansionTile(
                  key: listOfExpansion[index],
                  title: new Text(months[index]),
                  initiallyExpanded:
                      index == defaultExpandedItem ? true : false,
                  onExpansionChanged: (changed) {
                    int tmpExpanded = defaultExpandedItem;
                    if (changed) {
                      setState(() {
                        defaultExpandedItem = index;
                        listOfExpansion[index].currentState.expand();
                        listOfExpansion[tmpExpanded].currentState.collapse();
                      });
                    }
                  },
                  backgroundColor:
